...
case class Boom(x: Double, y: Double) extends Event {
    override def appliesTo(e: Entity): Boolean = e match {
        case ArcadeBaloon(_, x1, y1, _) => {
            ...
            val rooted:Double = Math sqrt(root)
...
case class ArcadeGameManager(val gamePoints: Int) extends Entity {
    ...
    override def handleEvent(global: MainEventLoop)(e: Event): (List[Entity], List[Event]) = e match {
        ...
        case BaloonEscaped => (List(ArcadeGameManager(gamePoints+ScreenDimensions.pointsBaloonEscaped)),List()) //TODO: Add points given by ScreenDimensions.pointsBaloonEscaped, no new events
        case BaloonBurst =>   (List(ArcadeGameManager(gamePoints+ScreenDimensions.pointsBaloonBurst)),List()) //TODO: Add points given by ScreenDimensions.pointsBaloonBurst, no new events
        case Stop => (List(),List()) // TODO: Empty list of entities and events to stop the trampoline.
        case _ => (List(ArcadeGameManager(gamePoints)),List()) //TODO: Return this entity unchanged and the empty list of events.
    }

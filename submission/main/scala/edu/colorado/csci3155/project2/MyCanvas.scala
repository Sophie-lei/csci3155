...
case class Polygon(val cList: List[(Double, Double)]) extends Figure {
    ...
    override def getBoundingBox: (Double, Double, Double, Double ) =
        return (cList.map(_._1).min, cList.map(_._1).max, cList.map(_._2).min, cList.map(_._2).max)
    ...
    override def translate(shiftX: Double, shiftY: Double): Polygon =
        return new Polygon(cList.map(x => (x._1 + shiftX, x._2 + shiftY)))

    override def rotate(angRad: Double): Polygon = {
        return new Polygon(cList.map(x => (x._1 * scala.math.cos(angRad) - x._2 * scala.math.sin(angRad), x._1 * scala.math.sin(angRad) + x._2 * scala.math.cos(angRad))))
    }
...
case class MyCircle(val c: (Double, Double), val r: Double) extends Figure {
    ...
    override def translate(shiftX: Double, shiftY: Double): MyCircle ={
        return new MyCircle((c._1 + shiftX, c._2 + shiftY), r)
    }

    override def rotate(angRad: Double): MyCircle = {
        return new MyCircle((c._1 * scala.math.cos(angRad) - c._2 * scala.math.sin(angRad), c._1 * scala.math.sin(angRad) + c._2 * scala.math.cos(angRad)), r)
    }
...
class MyCanvas (val listOfObjects: List[Figure]) {
    ...
    def getBoundingBox: (Double, Double, Double, Double) =
        return (listOfObjects.map(_.getBoundingBox._1).min, listOfObjects.map(_.getBoundingBox._2).max, listOfObjects.map(_.getBoundingBox._3).min, listOfObjects.map(_.getBoundingBox._4).max)
    ...
    val xShift = getBoundingBox._2 - myc2.getBoundingBox._1
    val yShift = ((getBoundingBox._4 + getBoundingBox._3) / 2) - ((myc2.getBoundingBox._4 + myc2.getBoundingBox._3) / 2)
    return overlap(myc2.translate(xShift, yShift))}

    //TODO: Write a function that will return a new MyCanvas object that places
    // all the figures in myc2 on top of the figures in this MyCanvas.
    // refer to the notebook documentation on how to perform this.
    def placeTop(myc2: MyCanvas): MyCanvas ={
    val xShift = ((getBoundingBox._2 + getBoundingBox._1) / 2) - ((myc2.getBoundingBox._2 + myc2.getBoundingBox._1) / 2)
    val yShift = getBoundingBox._4 - myc2.getBoundingBox._3
    return overlap(myc2.translate(xShift, yShift))}
    ...
    def rotate(angRad: Double): MyCanvas = {
        return new MyCanvas(listOfObjects.map(_.rotate(angRad)))
    }
